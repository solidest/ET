...
namespace ET.TestMap
{
    ...
    public class ModuleTestMap : ICommModule
    {
        ...
        {
            return new TestMapVM(LoadFile(mf, version) as TestMapData, mf);
        }

        public IViewDoc OpenNewFile(string name)
        {
            var ret = new TestMapVM(new TestMapData(ModuleTestMap.ModuleShowName), new ModuleFile(ModuleTestMap.ModuleKey, ModuleTestMap.ModuleShowName));
            ret.UpdateContent();
            return ret;
        }
